...
def _get_top_features_fallback(
    ...
    return [
        ...
        for d in top_features_global[:top_k]
    ]
...
def predict_single(features: Dict[str, Any]) -> Dict[str, Any]:
    ...
    missing = [c for c in feature_columns_all if c not in df_all.columns]
    if missing:
...
# 🔥 FIX: flip prediction
    ...
    if not top_features:
        top_features = _get_top_features_fallback(top_features_global)
